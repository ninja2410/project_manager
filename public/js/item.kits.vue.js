    ...
    methods: {
        ...
        validate() {
            ...
            this.prices.forEach((e, j) => {
                if (this.priceSale[j] === "0.00" || this.priceSale[j] === "null" || this.priceSale[j] === "") {
                    this.errores.push(`El precio de venta ${e.name} es requerido`);
                }
            });
            this.kitItems.forEach((e, j) => {
                if (e.quantity <= 0)
                    this.errores.push(`El producto ${e.item_name} no puede tener la cantidad de 0 en el kit`);
            });
        ...
        getCategory() {
            axios.get('/api/item-kit-vue/category')
                .then(response => {
        ...
        getItem() {
            this.loading=true;
            axios.get('/api/item-kit-vue/item')
                .then(response => {
                    this.items = response.data;
                    this.loading=false;
                })
        ...
        getPrices() {
            axios.get('/api/item-kit-vue/price')
                .then(response => {
        ...
        changeQuantity(index) {
            ...
            this.kitItems[index].valuePrices.forEach(i => {
                this.kitItems[index].total = parseFloat(parseFloat(this.kitItems[index].quantity) * parseFloat(this.kitItems[index].cost_price)).toFixed(2);
                this.kitItems[index].prices.push(parseFloat(i * this.kitItems[index].quantity).toFixed(2));
            });
            this.sumItems();
        },
        ...
        setVariables() {
            this.prices.forEach((element, j) => {
                ...
                this.priceSale[j] = parseFloat(0).toFixed(2);
                this.utility[j] = parseFloat(this.prices[j].pct).toFixed(2);
            });
        },
        sumPrice(index) {
            if (index != null)
        ...
        sumItems() {
            ...
            this.kitItems.forEach(element => {
                this.sumaCost = parseFloat(parseFloat(this.sumaCost) + parseFloat(element.total)).toFixed(2);                
            });
            ...
            this.kitItems.forEach(element => {
                element.prices.forEach((i, j) => {
                    this.subtotal[j] = parseFloat((parseFloat(this.subtotal[j])) + (parseFloat(i))).toFixed(2);
                });
            });
            if (this.flag === 0)
                this.sumPrice();
        ...
        addItemKit() {
            ...
            if (this.item !== "") {
                if (this.verificarExiste() === false) {
                    ...
                    newItem.prices = [];
                    newItem.valuePrices = [];
                    newItem.total = parseFloat(parseFloat(this.quantity) * parseFloat(this.item.cost_price)).toFixed(2);
                    axios.get('/api/item-kit-vue/itemPrice?item=' + this.item.id)
                        .then(response => {
                            let data = response.data;
                            data.forEach(element => {
                                newItem.prices.push(parseFloat(parseFloat(element.selling_price) * (newItem.quantity)).toFixed(2));
                                newItem.valuePrices.push(parseFloat(element.selling_price).toFixed(2));
                            });
        ...
        saveAll() {
            ...
            axios.post('/api/item-kit-vue/store', form, ajuste).then(function (response) {
