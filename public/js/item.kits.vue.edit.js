new Vue({
  ...
  methods: {
    ...
    getDataEdit() {
      axios
        ...
        .then(response => {
          ...
          this.nombreKit = this.kit.item_name;
          this.description = this.kit.description;
          this.categoria = this.kit.item_category;
          this.code = this.kit.upc_ean_isbn;
    ...
    setVariables() {
      /**Inicializo precios totales, utilidad */
      this.prices.forEach((element, j) => {
        this.subtotal[j] = parseFloat(this.price[j].selling_price).toFixed(2);
        console.log(this.price[j].selling_price+' '+this.price[j].pct)
    ...
    saveAll() {
      ...
      axios
        .post("/api/item-kit-vue/edit", form, ajuste)
