...
@section('title',trans('item.items_catalog'))
...
<div class="panel panel-primary">
  <div class="panel-heading clearfix">
    <h4 class="panel-title pull-left"> <i class="livicon" data-name="list-ul" data-size="16" data-loop="true" data-c="#fff" data-hc="white"></i>
      {{trans('item.items_catalog')}}
    </h4>
    <div class="pull-right">
      @if(in_array('ítems/create',$array_p))
      <a href="{{ URL::to('items/create') }}" class="btn btn-sm btn-default"><span class="glyphicon glyphicon-plus"></span> {{trans('item.new_item')}} </a>
      @endif
    </div>
  </div>            
  <div class="panel-body">
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <label>
          <b>{{trans('item.status')}}</b>
        </label>
        <select class="form-control" name="status" id="status">
          @foreach ($status as $key => $value)
          <option @if ($statusSelected==$value->id) selected @endif value="{{$value->id}}">{{$value->name}}</option>
          @endforeach
        </select>
      </div>
      <div class="col-md-3">
      ...
      <table class="table table-striped table-bordered display" id="table1">
        <thead>
          <tr>
            ...
            <th style=" width:8%">{{trans('item.size')}}</th>
...
var table = $('#table1').DataTable({
  "ajax":{
    "method":"GET",
    "url":"items/index_ajax_by_price",
    "data": function ( d ) {
      d.price_id = cleanNumber($('#price_id').val());
      d.status = cleanNumber($('#status').val());
    },
    "dataSrc": "",
    "headers": {
  ...
  { "data": "id", render: function (data,type,row) {
    if(row.status==1){
      if(row.existencia>0) {
        return '<a class="btn btn-small btn-success btn-xs" href="items/detail/' + data + '" data-toggle="tooltip" data-original-title="Detalles"><span class="glyphicon glyphicon-eye-open\"></span></a>' +
        '<a class="btn btn-small btn-info btn-xs" href="items/' + data + '/edit" data-toggle="tooltip" data-original-title="Editar"><span class="glyphicon glyphicon-edit\"></a>';        
      }
      else {
        return '<a class="btn btn-small btn-success btn-xs" href="items/detail/' + data + '" data-toggle="tooltip" data-original-title="Detalles"><span class="glyphicon glyphicon-eye-open\"></span></a>' +
        '<a class="btn btn-small btn-info btn-xs" href="items/' + data + '/edit" data-toggle="tooltip" data-original-title="Editar"><span class="glyphicon glyphicon-edit\"></a>' +
