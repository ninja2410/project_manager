...
<div class="row">
    ...
    <div class="col-md-3">
        <center><label for=""><b>{{trans('project.stage')}}</b></label></center>
        <select class="form-control" id="stage" name="stage">
            <option value="Todo" {{ ($stage == 'Todo') ?  'selected="selected"' : '' }}>Todos</option>
            @foreach($stages as $index => $value)
                <option value="{{$value->id}}" {{ ($stage == $value->id) ?  'selected="selected"' : '' }}>{{$value->name}}</option>
            @endForeach
        </select>
    </div>
    <div class="col-md-2">
