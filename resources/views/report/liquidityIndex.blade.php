...
@section('title','Indice de liquidez por clientes')
@section('page_parent','Indice de liquidez')
...
<section class="content">
	<div class="row">
        ...
        {{-- {!! Form::open(array('url'=>'reports/customers_pending_to_pay')) !!} --}}
          ...
          {!! Form::open(['url' => url('liquidityIndex'),
                    ...
                    <tbody>
                      ...
                      $promedio=0;?>
                       ...
                       <tr>
                         ...
                         <td>{{$value->CUOTAS}}</td>
                         <td>{{$value->ATRASOS}}</td>
                         <td>{{number_format(($value->CUOTAS/($value->CUOTAS+$value->ATRASOS))*100, 2)}}%</td>
                        <?php
                        $acumulador++;
                        $cuotas+=$value->CUOTAS;
                        $atrasos+=$value->ATRASOS;
                        $promedio+=$value->CUOTAS/($value->CUOTAS+$value->ATRASOS);
                          ?>
                       </tr>
                       @endforeach
                       <tr>
                         <td></td>
                         <td>TOTAL</td>
                         <td style="text-align:right;background-color:#012814;color:white;">{{$cuotas}}</td>
                         <td style="text-align:right;background-color:#012814;color:white;">{{$atrasos}}</td>
                         <td style="text-align:right;background-color:#012814;color:white;">Promedio: @if ($promedio>0)
                           {{number_format($promedio/$acumulador*100, 2)}}%
                           @else
                             0 %
                         @endif</td>
                       </tr>
