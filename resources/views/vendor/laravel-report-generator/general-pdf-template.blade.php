...
<html lang="en">
	...
	<body>
		...
		<div class="wrapper">
		    <div class="pb-5">
				<div class="middle pb-20 title">
					<img src="{{ public_path('images/system/logo.png') }}" alt="Cacao-ERP" style="max-width: 150px;float: left;">
			        {{ $headers['title'] }}
			    </div>
    			@if ($showMeta)
				<div class="head-content pt-10">
		    ...
		    <div class="content">
		    	<table width="100%" class="table">
					...
					@foreach($query->take($limit) as $result)
						<?php
							...
							if ($groupByArr) {
				    			...
				    			if ($isOnSameGroup === false) {
		                            ...
		                            if ($showNumColumn || $grandTotalSkip > 1) {
		                                echo '<td colspan="' . $grandTotalSkip . '" style="text-align: center"><b>TOTAL</b></td>';
		                            }
									$dataFound = false;
	    							foreach ($columns as $colName => $colData) {
	    								if (array_key_exists($colName, $showTotalColumns)) {
					...
					@if ($showTotalColumns != [] && $ctr > 1)
						<tr class="bg-black f-white">
                            @if ($showNumColumn || $grandTotalSkip > 1)
                                <td colspan="{{ $grandTotalSkip }}" style="text-align:center;"><b>TOTAL =></b></td> {{-- For Number --}}
                            @endif
							<?php $dataFound = false; ?>
							@foreach ($columns as $colName => $colData)
								@if (array_key_exists($colName, $showTotalColumns))
