...
@section('title', trans('budget.clone'))
...
{{--            </div>--}}
            <div class="row">
              <div class="col-lg-6">
                <div class="form-group">
                  {!! Form::label('lblproject', trans('budget.project')) !!}
                  <div class="input-group">
                    <span class="input-group-addon"><li class="glyphicon glyphicon-folder-close"></li></span>
                    <select name="project_id" id="project_id" class="form-control" form="frmSend">
                      @foreach($projects as $project)
                        <option value="{{$project->id}}">{{$project->name}} | {{$project->type->name}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
            ...
            <div>
              ...
              {!! Form::open(['url' => 'project/budget/clone/'.$budget->id, 'method' => 'POST', 'id'=>'frmSend']) !!}
            ...
            <div class="row">
              @include('partials.buttons_withmodal', ["cancel_url" => url('project/'.$budget->project_id.'/budget/')])
            </div>
            <div>
